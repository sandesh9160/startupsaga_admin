...
export default function DashboardPagesPage() {
    ...
    // Group pages
    const coreSlugs = ['home', 'homepage', 'categories', 'cities', 'startups', 'stories'];
    const mainPages = pages.filter(p => coreSlugs.includes(p.slug.toLowerCase()));
    const customPages = pages.filter(p => !coreSlugs.includes(p.slug.toLowerCase()));

    const filteredMain = mainPages.filter(p =>
        p.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        p.slug.toLowerCase().includes(searchQuery.toLowerCase())
    );

    const filteredCustom = customPages.filter(p => !p.is_system).filter(p =>
        p.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        p.slug.toLowerCase().includes(searchQuery.toLowerCase())
    );
            ...
            <div className="max-w-[1400px] mx-auto p-4 lg:p-8 space-y-8 flex flex-col min-h-[85vh]">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl bg-zinc-50 border border-zinc-100 shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h1 className="text-xl font-bold tracking-tight text-zinc-900">Site Pages</h1>
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                        ...
                        <h2 className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Main Pages</h2>
                    </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {loading ? (
                                        ...
                                        <tr>
                                            <td colSpan={5} className="py-10 text-center text-zinc-400 text-xs italic">No main pages found</td>
                                        </tr>
                                    ) : (
                                        filteredMain.map((p) => (
                                            <tr key={p.id} className="hover:bg-slate-50/60 transition-colors group">
                                                ...
                                                <td className="px-4 py-3.5 text-center">
                                                    <Badge variant="secondary" className="text-[9px] px-2 py-0.5 font-bold rounded-lg bg-emerald-50 text-emerald-600 border-0 uppercase">
                                                        Core
                                                    </Badge>
                                                </td>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                        ...
                        <h2 className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Custom Pages</h2>
                    </div>
                    <Card className="border border-slate-100 shadow-sm bg-white rounded-2xl overflow-hidden">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                        ...
                                        <tr>
                                            <td className="py-20 text-center">
                                                <div className="flex flex-col items-center justify-center text-center">
                                                    ...
                                                    <p className="text-xs font-bold text-slate-400">No custom pages found</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                                ...
                                                <td className="px-5 py-3.5">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="flex items-center gap-2 mt-0.5">
                                                                    <span className="text-[10px] text-zinc-400 font-mono">/{p.slug}</span>
                                                                    <span className="text-[8px] px-1.5 py-0 rounded bg-slate-100 text-slate-500 font-bold uppercase tracking-tighter">
                                                                        {p.status}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="flex items-center gap-1 opacity-10 group-hover:opacity-100 transition-opacity">
                                                            <button
                                                                onClick={() => window.open(`${process.env.NEXT_PUBLIC_FRONTEND_URL || 'http://localhost:3000'}/${p.slug}`, "_blank")}
                                                                className="h-7 w-7 rounded-lg text-slate-400 hover:text-sky-600 hover:bg-sky-50 flex items-center justify-center transition-all"
                                                            >
                                                                <Eye size={12} />
                                                            </button>
                                                            <button
                                                                onClick={() => router.push(`/dashboard/site-pages/${p.id}/edit`)}
                                                                className="h-7 w-7 rounded-lg text-slate-400 hover:text-purple-600 hover:bg-purple-50 flex items-center justify-center transition-all"
                                                            >
                                                                <Edit size={12} />
                                                            </button>
                                                            {!p.is_system && (
                                                                <button
                                                                    ...
                                                                    className="h-7 w-7 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 flex items-center justify-center transition-all disabled:opacity-40"
                                                                >
                                                                    <Trash2 size={12} />
                                                                </button>
                                                            )}
